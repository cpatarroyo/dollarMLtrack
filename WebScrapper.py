...
for name in results:
    ...
    if name in listyester:
        yesterpr = float(((float(results[name])-float(listyester[name]))/float(listyester[name]))*100)
    else:
        yesterpr = None
    if name in listweek:
        weekpr = float(((float(results[name])-float(listweek[name]))/float(listweek[name]))*100)
    else:
        weekpr = None
...
for name in curResults:
    ...
    if name in curyester:
        yestercur = float(((float(curResults[name])-float(curyester[name]))/float(curyester[name]))*100)
    else:
        yestercur = None
    if name in curweek:
        weekcur = float(((float(curResults[name])-float(curweek[name]))/float(curweek[name]))*100)
    else:
        weekcur = None
